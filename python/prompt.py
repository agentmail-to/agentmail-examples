...
The agent passes Claude the email body + each PDF / image attachment as a
vision document, and Claude calls one of two tools:
  - extract_invoice(vendor_name, invoice_number, amount, currency, due_date, ...)
  - cannot_extract(reason)
"""

import os

SYSTEM_PROMPT_TEMPLATE = """You read incoming email + attached invoices and extract structured fields. The email arrives in {company} accounts payable inbox at {inbox_email}.
...
def build_system_prompt(inbox_email: str) -> str:
    ...
    return SYSTEM_PROMPT_TEMPLATE.format(
        ...
        company=os.getenv("COMPANY_NAME", "the buyer's"),
